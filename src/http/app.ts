...
export interface CreateHttpAppOptions {
  /**
   * Include a static asset fallback so the Express server serves the built UI.
   * Disable when another platform (such as Vercel) serves the static output.
   */
...
export async function createHttpApp(options: CreateHttpAppOptions = {}): Promise<HttpApp> {
  ...
  const attachLogStream = (req: Request, res: Response) => {
    res.writeHead(200, {
  ...
  app.get("/api/logs/stream", attachLogStream);
  app.get("/events", attachLogStream);
  ...
  const staticInfo = includeStaticFallback ? resolveStaticDir() : null;
  if (staticInfo) {
