...
export async function POST(req: NextRequest) {
  ...
  const auth = await validateApiKey(req.headers, 'social:*', 'social.render', 'social_queue');
  if (!auth.ok) {
  ...
  await logEvent({
    source: 'social',
    kind: 'social.render.requested',
    title: `Render ${body.template}`,
    meta: { template: body.template, payload: body.payload }
  });
  return Response.json({ ok: true, status: 'render-queued' });
}
